...
export function BookingStageOne({ barbershop, services }: BookingStageOneProps) {
  ...
  useEffect(() => {
    ...
    (window as unknown as { __recaptchaLoaded?: () => void }).__recaptchaLoaded = () => setCaptchaReady(true);
    return () => {
      delete (window as unknown as { __recaptchaLoaded?: () => void }).__recaptchaLoaded;
    };
  }, []);

  useEffect(() => {
    if (!RECAPTCHA_SITE_KEY || !captchaReady || !captchaContainerRef.current) return;
    const g = (window as unknown as { grecaptcha?: { render: (el: HTMLElement, opts: { sitekey: string }) => number; getResponse: (id: number) => string; reset: (id?: number) => void } }).grecaptcha;
    if (!g) return;
    if (captchaWidgetIdRef.current != null) return;
    captchaWidgetIdRef.current = g.render(captchaContainerRef.current, { sitekey: RECAPTCHA_SITE_KEY });
  }, [captchaReady]);

  const onConfirmBooking = handleSubmit(async (data) => {
    if (!selectedService || !selectedDate || !selectedSlot) return;
    if (!RECAPTCHA_SITE_KEY) {
      setSubmitError('Captcha não configurado. Não é possível agendar.');
      return;
    }
    const g = (window as unknown as { grecaptcha?: { getResponse: (id: number) => string; reset: (id?: number) => void } }).grecaptcha;
    const token = g && captchaWidgetIdRef.current != null ? g.getResponse(captchaWidgetIdRef.current) : '';
    if (!token) {
      setSubmitError('Marque o captcha para continuar.');
      return;
    }
    ...
    try {
      ...
      if (RECAPTCHA_SITE_KEY && g && captchaWidgetIdRef.current != null) {
        g.reset(captchaWidgetIdRef.current);
      }
      ...
      const message =
        err instanceof Error && (err as Error).message
                ...
                <div className="rounded-2xl border border-zinc-200 bg-white p-4">
                  ...
                  <form onSubmit={onConfirmBooking} className="space-y-4">
                    ...
                    {RECAPTCHA_SITE_KEY && (
                      <>
                        <Script
                          src="https://www.google.com/recaptcha/api.js?onload=__recaptchaLoaded&render=explicit"
                          strategy="lazyOnload"
                        />
